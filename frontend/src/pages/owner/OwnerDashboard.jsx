...
const HostelCard = ({ hostel, isSuspended }) => {
  ...
  const license = hostel.business_licenses?.[0];
  const isLicenseRejected = license?.status?.label === 'Rejected';
  const status = isLicenseRejected ? 'Rejected' : (hostel.listing_status?.label ?? 'Draft');
      ...
      {status === 'Rejected' && license?.rejection_reason && (
        <div className="bg-amber-50 border border-amber-100 rounded-xl px-3 py-2.5">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-amber-600">License Rejected</p>
              <p className="text-xs text-amber-700 mt-0.5">{license.rejection_reason}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        {(status === 'Disabled' || status === 'Rejected') && (
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-white text-xs font-semibold rounded-lg transition ${
              status === 'Disabled' ? 'bg-red-500 hover:bg-red-600' : 'bg-amber-500 hover:bg-amber-600'
            }`}
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
