...
const useCountdown = (createdAt) => {
  const expiry = useMemo(() => new Date(createdAt).getTime() + 24 * 60 * 60 * 1000, [createdAt]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        {step === 'choose' && (
          <div className="space-y-3">
            ...
            >
              ...
              <div>
                <p className="font-semibold text-gray-800 text-sm">I will pay at the property</p>
                <p className="text-xs text-gray-400">Notify the owner, pay in person</p>
              </div>
            </button>
          </div>
        )}
        ...
        {step === 'upload' && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={transactionId} onChange={(e) => setTxId(e.target.value)} placeholder="e.g. TXN123456"
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
        ...
        {step === 'property' && (
          <div className="text-center py-4 space-y-4">
            ...
            <p className="text-sm text-gray-600">The owner has been notified. Please pay <span className="font-semibold text-gray-800">in person at the property</span> within the remaining time. Your booking will be confirmed once cash is received.</p>
            <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-teal-500 hover:bg-teal-600 text-white text-sm font-semibold transition">Got it</button>
          </div>
        )}
      </div>
    </div>
...
const BookingCard = ({ booking, onPayNow }) => {
  const { expired, display } = useCountdown(booking.created_at);
  const isPending   = booking.status?.label === 'Pending';
  const isCancelled = booking.status?.label === 'Cancelled';
    ...
    <div className={`bg-white rounded-2xl border shadow-sm p-5 transition ${isCancelled ? 'opacity-60 border-gray-100' : 'border-gray-100 hover:shadow-md'}`}>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{hostel?.township?.name} · {hostel?.address}</p>
        </div>
      ...
      {latestPayment && (
        <div className="mb-3 px-3 py-2 bg-gray-50 rounded-xl text-xs text-gray-500 flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded-full font-semibold ${STATUS_STYLES[latestPayment.status?.label] ?? 'bg-gray-100 text-gray-600'}`}>
      ...
      {isPending && (
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-gray-100">
          <div className={`flex items-center gap-2 text-sm font-medium ${expired ? 'text-red-500' : 'text-amber-600'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            {expired ? 'Expired' : `Time remaining: ${display}`}
          </div>
          {!expired && (
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 py-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/" className="text-sm text-teal-600 hover:underline">Browse Hostels →</Link>
        </div>

        {successMsg && (
          <div className="mb-5 p-4 bg-green-50 border border-green-200 rounded-xl text-sm text-green-700 flex items-center gap-2">
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {bookings.map((b) => (
              <BookingCard key={b.id} booking={b} onPayNow={setPayTarget} />
            ))}
          </div>
        )}
      </main>
      ...
      {payTarget && (
        <PayNowModal
          ...
          onSuccess={(msg) => { setPayTarget(null); setSuccessMsg(msg); }}
        />
      )}
    </div>
  );
